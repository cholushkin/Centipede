...
public class Centipede : BoardEntityBase
{
    public class Segment
    {
        ...
        public void SetBoardPosition(Vector2Int pos, bool removePrev)
        {
            ...
            _visualSegment.SetActive(_centipede.Board.CellAccessor.IsInside(pos)); // don't draw outside the field (for spawning)
        ...
        private readonly Centipede _centipede;
    }
    ...
    {
        Debug.LogFormat("Cent: damage at {0}, SPLITTING!", boardPosition);
        Split(boardPosition);
    ...
    public void Init(float speed, Segment head, Segment tail, BoardController board, Vector2Int moveDirection)
    {
        ...
        _head = head;
        _tail = tail;
        _head.GetVisualSegment().GetComponent<SpriteRenderer>().sprite = Visual.MultiSprite.GetSprite(0); // set head sprite
